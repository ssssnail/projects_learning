...
# 
# 回答：这些图像中，监测到人脸占比为0.99,又跑了一次成1了。
# - `dog_files` 的前100张图像中，能够检测到**人脸**的图像占比多少？
# 
# 回答：这些图像中，监测到人脸占比为0.12
# 
...
for path in human_files_short:  
    result = face_detector(path)
    #human_result.append(result)
    if(result == True):
        human_sum  = human_sum + 1
for path in dog_files_short: 
    result = face_detector(path)
    #dog_result.append(result)
    if (result == True):
        dog_sum = dog_sum + 1
    
print('human of human :',human_sum / len(human_files_short))
print('dog of human :',dog_sum / len(dog_files_short))
...
# __回答:__
# 
# 这是不合理的，用户没有必要配合软件。如果只是人脸的话，我能想到的有两点。
# 
# 1.去匹配关键点，比如有鼻子、眼睛、耳朵。当然动物也都是这样，使用一些人的鼻子、眼睛、耳朵训练
# 
# 2.训练图像中包含大量的模糊人脸信息，并加强对这些图像的训练和特征提取。
# 
# 
...
# 选作暂时先不做了，因为我和朋友有个约定，所以想尽快先把项目过了
# 
# 后边的选作也都没有做，还有一个疑惑的，我回头会再提交一次选做的和一些疑惑问题
# 
# 但是，听说又不能再提交了，如果过了的话，那我如果过了，还是自己再把选作做一下
# 
# 感谢
...
for path in dog_files_short:
    if dog_detector(path):
        dog_sum2 = dog_sum2 + 1
print('dogs in human_files_short :',dog_sum1 / (len(human_files_short)))
print('dogs in dog_files_short :',dog_sum2 / (len(dog_files_short)))
...
# __回答:__ 
# 
# 1.我是根据上文的提示（网络结构的图），进行反推回去，然后搭建的网络，shape，卷积核大小，pool_size，strides 都是可以计算得到的
# 
# a.卷积层：根据输出层的shape和参数数量，决定了卷积核的大小，使用relu是因为我不知道使用什么好（真心的）这个需要给点建议。
# 
# b.最大池化：还是根据输出shape，计算了pool_size和strides，选择最大池化是因为网络结构就是这样，最大池化我认为可以更加明显的采集图像中比较明显的
# 特征（即和其他区域相差较大的特征）
# 
# c.卷基层：同上，网络继续加深，提取图像更多、更深层次的特征
# 
# d.最大池化：同上
# 
# e.全局平均池化：代替了全连接层，降低了参数量，将特征压缩进一个向量里边，输出给Dense层用作分类
# 
# f.dense: 进行最后的分类，133为分类数，激活函数使用softmax是因为多分类
# 
# 2.就是因为有卷积层的存在，我们可以提取图像的特征图，计算特征图与标签的相关性。CNN在图像识别中表现本身就是很好，哈哈。更深的原因，可能需要您大概讲解一下了 感谢
# 
# 
...
# __回答:__ 
# 
# 回答：这个模型使用迁移模型，我拿来了resnet在imagenet上训练的的瓶颈特征图，全连接层之前的最后一层输出（已经具有很强的辨识度），因为我们的分类其实已经在这imagenet1000个分类里边了，所以拿最后一层也是没有问题的，如果我们要预测的图形不在这1000个分类里边，那我们可能需要拿再低级一点的特征图，更抽象一些的特征图，然后后边的层需要用我们自己的数据进行训练。添加的全局平均池化，代替了全连接层，降低的参数量。
# 
# 使用这个模型的原因就是因为示例中已经用了VGG16，然后VGG19也是差不多的，而且表现并不好，所以尝试了resnet和Incepiton（在本地已经跑过了），resnet已经有点过拟合？（这块有点不明白了，因为我跑几次之后，valid loss 是增加的，也就是跑一个epoch是最好的，但是这里跑完又有点正常，总之这个模型很棒，哈哈）
# 
# 
# 
...
resnet_model.fit(train_resnet,train_targets,validation_data=(valid_resnet,valid_targets),epochs=5,
...
def play(img_path):
    ...
    if face_detector(img_path):
        ...
        plt.axis('on')
        plt.imshow(img)
        print('hello ,human! you look like a ',prediction(img_path))
    elif dog_detector(img_path):
        plt.title('hi dog')
        plt.axis('on')
        plt.imshow(img)
        print('hello, dog !  you look like a ',prediction(img_path))
    else:
        plt.title('hi monster')
        plt.axis('on')
        plt.imshow(img)
        print('sorry :(  you put the wrong path')
...
# 
# 因为限制于本地环境，我跑预测都很困难 ：（ ，没有做太多测试  应该是没有问题的。
# 
# <a id='question11'></a>  
# 
# ### __问题 11:__
...
# 1. 输出结果比你预想的要好吗 :) ？或者更糟 :( ？
# 
# 因为我在本地使用inception·这里使用了resnet,本身模型优于VGG，而且只训练了3个epoch，准确率已经到了80。结果在我预想的之内，可以准确的判断人脸、狗和非人和狗。
# 2. 提出至少三点改进你的模型的想法。
# 
# 1.因为一些原因没有做数据增强，如果做数据增强的话，应该会更好一些
# 
# 2.如果只针对这个模型的话，跑预测的速度太慢了，每次都会计算1000个分类的概率，可以考虑调整数据集，只针对人、狗来训练。
# 
# 3.增加训练次数。
...
## 自由地使用所需的代码单元数吧
play('images/test.jpg')

#6张图在本地跑的很困难， 跑了3张好像， 电脑太破啦！！


# In[40]:


play('images/test1.jpg')
...
play('images/test_human.jpg')


# In[42]:


play('images/test_human1.jpg')


# In[43]:


play('images/test_dog.jpg')


# In[44]:


play('images/test_dog1.jpg')
