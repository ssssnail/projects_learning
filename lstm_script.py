...
def create_lookup_tables(text):
   ...
   # vocab = sorted(counter,key=counter.get,reverse=True)
    #print(type(vocab))
    #vocab_to_int = {word:index for index,word in enumerate(vocab,0)}
    #int_to_vocab = {index:word for index,word in enumerate(vocab,0)}
...
# #### 所以如果使用counter（也就是词出现的频率）来代替单词的话，和使用set（也就是他们的索引）代替单词，对于模型没有区别
# 
# #### 只要单词可以对应的转换为数字就可以对，是吗？
...
def token_lookup():
    ...
    """
    # TODO: Implement Function
    token_dict = {'.':'||period||',',':'||comma||','"':'||quotation_mark||',';':'||semicolon||','!':'||exclamation_mark||','?':'||question_mark||',
...
def get_init_cell(batch_size, rnn_size):
    ...
    #print(type(lstm))
    cell = tf.contrib.rnn.MultiRNNCell([lstm]*layer_num)
...
def build_nn(cell, rnn_size, input_data, vocab_size, embed_dim):
    ...
    """
    input_data_embed = get_embed(embed_dim=embed_dim,input_data=input_data,vocab_size=vocab_size)
    #print(input_data_embed.shape)
    outputs,final_state = build_rnn(cell=cell,inputs=input_data_embed)
    #print(outputs.shape)
    logits = tf.contrib.layers.fully_connected(outputs,vocab_size,activation_fn=None)
    
    #构建最后一层输出层
    #output_h = tf.concat(outputs,axis = 1)#将每个时间点的lstmoutput 拼接在一起 得到最后一个时间点的lstm output,即为输出层的输入
    #print(output_h.shape)
    #x = tf.reshape(outputs,[-1,rnn_size])# 将输入reshape为矩阵，列为rnn size 隐藏层的行，方便矩阵乘法
    #print(x.shape)
    #hidden_w = tf.Variable(tf.truncated_normal((rnn_size,vocab_size),stddev=0.1))
   # hidden_b = tf.Variable(tf.zeros(vocab_size))
    #print(hidden_w.shape)
    #logits = tf.matmul(x,hidden_w) + hidden_b
   # print(logits.shape)
    # TODO: Implement Function
...
# #### 之前我是fully_connected()是没有加激活函数（完全没有写），但是代码还是可以跑通的
# 
# #### 您提示说需要加上， 这里的区别是什么呢？ 
...
# #### 有个问题： get_batches 这个函数，实现起来有点困难了，想请教一下，我下边实现的时候，其实是找到的网上的一种解决方案。
# #### 按照之前课程里讲的实现方法，应该如何切割原来的数据,也就是如何保证for循环的时候，可以得到y的整个batch（y往后移了一列）。
# #### int_text = int_text[:num_batches * per_batch_text_num + batch_size]  （我是这样写的，但是不对）
# 
# 
# #### 我的思路是：在切原来的数据的时候，多切一个batche_size ，然后reshape的时候，会多出一列，然后这一列刚好可以给y的最后一个batch，但是实}
# 
# #### 现起来又不对。想问一下，感谢。 
...
#def get_batches(int_text, batch_size, seq_length):
   ...
   # per_batch_text_num = batch_size * seq_length
    
  #  int_text = int_text[:num_batches * per_batch_text_num + batch_size] #取整个batche数量，剩余的数据抛弃
   # int_text = np.array([int_text])
    #print(int_text.shape)
   # int_text = int_text.reshape(batch_size,-1) #将输入数据 reshape为batchesize 行
   ...
   # batches = []
    ...
    #        x = int_text[:,i:i + seq_length]
     ...
     #       batches.append(np.concatenate((x,y)))
...
# #### 有个问题：应该如何调整超参数，或者初始值应该怎么选，我做的时候，有点像无头苍蝇。希望能得到一些建议。（这个您已经回答过了） 
# 
# #### 还有一点就是  有时候会报这个错误，256,512 都遇到过，是rnn_size和 embed_dim的 和 ，要等于一个值。  有时候却不用，不知道什么情况
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    #max_index = np.where(probabilities == np.max(probabilities,axis=0))
    
   # a = max_index[0][0]  #max_index type为 array，max_index[0] 为tuple， max_index[0][0]表示获取array中第一个tuple中的第一个值
    #print(a)
    #pred_word = str(int_to_vocab.get(a))
...
# #### 这个地方也挺奇怪的，打印的时候之前会生成有一堆数字，好像得到的概率一样，但是这次也没有了，为什么。
# 
# #### 修改之后的效果确实好很多了，感谢 感谢，真心感谢！！
